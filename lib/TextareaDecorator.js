...
function TextareaDecorator( textarea, parser ){
	...
	var color = function( input, output, parser ){
		...
		// find inputPt, the starting position in the input text which has
		// not yet been tokenized
		var inputPt = 0;
		// and outputPt, the corresponding position in the output nodes which
		// requires modification
		var outputPt;
		var lastTokenLength = 0;
		for( outputPt = 0; outputPt < oldTokens.length; outputPt++ ){
			var oldToken = oldTokens[outputPt];
			if( oldToken === input.substr( inputPt, oldToken.length ) ){
				lastTokenLength = oldToken.length;
				inputPt += lastTokenLength;
			} else {
				break;
			}
		}
