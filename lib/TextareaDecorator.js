...
function TextareaDecorator( textarea, parser ){
	...
	// coloring algorithm
	// m is tokenized input, n is output node
	var color = function( m, output ){
		var n = output.childNodes;
		var i, j, mp, np;
		...
		var insertionPt = n[i];
		var insert = ( i === n.length ) ?
			function(e){ output.appendChild(e); } :
			function(e){ output.insertBefore(e,insertionPt); };
		// this is why we can't mess with the br
		for( ; i <= mp; i++ ){
	...
	api.update = function(){
		...
		if( input ){
			color( parser.tokenize(input), output );
			// determine the best size for the textarea
			var lines = textarea.value.split('\n');
			var maxlen = 0, curlen;
			for( var i = 0; i < lines.length; i++ ){
				curlen = lineLength(lines[i]);
